...
if (isset($_GET['conId'])) {
    ...
    if (mysqli_query($conn, $delete_contact)) {
        header("Location:{$rootName}/admin/contact.php");
    } else {
...
if (isset($_GET['skId'])) {
    $skId = $_GET['skId'];
    $delete_sk = "DELETE FROM `skills` WHERE  sk_id=$skId";
    if (mysqli_query($conn, $delete_sk)) {
        header("Location:{$rootName}/admin/skills.php");
    } else {
        echo "<script> alert('skill Not Deleted ') </script>";
    }
}
